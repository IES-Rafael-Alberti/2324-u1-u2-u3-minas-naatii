...
class TestColocarminar:

    # The function correctly handles the case where the number of mines is greater than the number of cells on the board.
    def test_too_many_mines(self):
        global NUMERODEMINAS
        NUMERODEMINAS = FILAS * COLUMNAS + 1
        colocarMinar()
        assert len(listaMinas) == FILAS * COLUMNAS